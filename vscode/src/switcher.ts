...
class Switcher {
  ...
  protected toTest(): string {
    return this.path.replace(this.implementationMatcher, this.implementationReplacer);
  }
  ...
  private get implementationReplacer(): string {
    const replacement = this.configuration(`${this.languageId}.implementationReplacer`);
    if (!replacement) {
  ...
  private configuration(section: string): string | undefined {
    const configuration = vscode.workspace.getConfiguration('Phy');
    ...
    .then(
      document => {
        vscode.window.showTextDocument(document).then(
          () => vscode.commands.executeCommand("workbench.action.keepEditor", document.uri),
...
const createFile = (targetFile: string) => {
  vscode.workspace.fs.writeFile(vscode.Uri.file(targetFile), new Uint8Array()).then(
    () => openFile(targetFile),
    () =>
      vscode.window.showErrorMessage(
        `Could not create file ${targetFile}.`
      )
  );
};
